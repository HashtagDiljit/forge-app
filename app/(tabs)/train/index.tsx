...
function OverviewView() {
  ...
  const preloaded = useMemo<WorkoutTemplate[]>(() =>
    WORKOUT_TEMPLATES.map((t) => ({ ...t, createdAt: 0, exercises: t.exercises.map((e, i) => ({ id: `${t.id}-${e.exerciseId}`, exerciseId: e.exerciseId, defaultSets: e.defaultSets, defaultReps: e.defaultReps, restSeconds: e.restSeconds, order: i })) })), []);

  const allTemplates = useMemo(() => [...preloaded, ...templates], [preloaded, templates]);

  const startFromTemplate = (tpl: WorkoutTemplate) => {
    if (activeWorkout) { Alert.alert('Workout Active', 'Finish or discard your current workout first.'); return; }
  ...
  return (
    <ScrollView style={styles.scroll} contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
      ...
      <View style={styles.quickActions}>
        <Button label="Quick Start" onPress={startBlank} size="lg" fullWidth leftIcon={<MaterialCommunityIcons name="plus" size={20} color={Colors.background.primary} />} />
        <Button label="Plate Calc" onPress={() => setShowPlateCalc(true)} variant="secondary" leftIcon={<MaterialCommunityIcons name="calculator" size={18} color={Colors.text.primary} />} />
      </View>
      <Text variant="caption" color="secondary" style={{ marginBottom: Spacing.lg, marginTop: -Spacing.sm }}>{format(new Date(), 'EEEE, MMMM d')}</Text>
      <View style={styles.sectionHeader}>
        <Text variant="subtitle">Templates</Text>
        <TouchableOpacity style={styles.addBtn} onPress={() => setShowCreateTemplate(true)}>
      ...
      {allTemplates.map((tpl) => (
        <TemplateCard
          ...
          onDelete={!tpl.isPreloaded ? () => Alert.alert('Delete Template', `Delete "${tpl.name}"?`, [{ text: 'Cancel', style: 'cancel' }, { text: 'Delete', style: 'destructive', onPress: () => deleteTemplate(tpl.id) }]) : undefined}
        />
      ))}
      <PlateCalculator visible={showPlateCalc} onClose={() => setShowPlateCalc(false)} />
      <TemplateEditorModal visible={showCreateTemplate} template={null} onClose={() => setShowCreateTemplate(false)} onSave={(t) => { addTemplate(t); setShowCreateTemplate(false); }} />
      <TemplateEditorModal visible={!!editingTemplate} template={editingTemplate} onClose={() => setEditingTemplate(null)} onSave={(t) => { updateTemplate(t.id, t); setEditingTemplate(null); }} />
      <View style={{ height: 100 }} />
    </ScrollView>
  );
}

function TemplateCard({ template, onStart, onEdit, onDelete }: { template: WorkoutTemplate; onStart: () => void; onEdit?: () => void; onDelete?: () => void }) {
      ...
      <TouchableOpacity onPress={onStart} activeOpacity={0.7}>
        <View style={styles.templateHeader}>
          <View style={{ flex: 1 }}>
            ...
            {template.isPreloaded && <Badge label="Built-in" color={Colors.text.secondary} />}
          </View>
          <TouchableOpacity style={[styles.startChip, { backgroundColor: `${BLUE}20`, borderColor: `${BLUE}40` }]} onPress={onStart}>
        ...
        {exercises.length > 0 && (
          <View style={{ marginTop: Spacing.xs }}>
            ...
            {exercises.length > 4 && <Text variant="caption" color="secondary">+{exercises.length - 4} more</Text>}
          </View>
        )}
      </TouchableOpacity>
      {(onEdit || onDelete) && (
        <View style={styles.templateActions}>
          {onEdit && <TouchableOpacity style={styles.templateActionBtn} onPress={onEdit}><MaterialCommunityIcons name="pencil-outline" size={15} color={Colors.text.secondary} /><Text variant="caption" style={{ marginLeft: 3 }}>Edit</Text></TouchableOpacity>}
          {onDelete && <TouchableOpacity style={styles.templateActionBtn} onPress={onDelete}><MaterialCommunityIcons name="trash-can-outline" size={15} color={Colors.accent.secondary} /><Text variant="caption" style={{ color: Colors.accent.secondary, marginLeft: 3 }}>Delete</Text></TouchableOpacity>}
        </View>
      )}
    </Card>
  );
...
function TemplateEditorModal({ visible, template, onClose, onSave }: { visible: boolean; template: WorkoutTemplate | null; onClose: () => void; onSave: (t: WorkoutTemplate) => void }) {
  const [name, setName] = useState(template?.name ?? '');
  const [exercises, setExercises] = useState<TemplateExercise[]>(template?.exercises ?? []);
  const [showExPicker, setShowExPicker] = useState(false);
  React.useEffect(() => { if (visible) { setName(template?.name ?? ''); setExercises(template?.exercises ?? []); } }, [visible, template]);
  const handleSave = () => {
    if (!name.trim()) { Alert.alert('Name required'); return; }
    onSave({ id: template?.id ?? generateId('tpl'), name: name.trim(), exercises, isPreloaded: false, createdAt: template?.createdAt ?? Date.now() });
  };
  const addEx = (ex: typeof EXERCISE_LIBRARY[0]) => {
    setExercises((prev) => [...prev, { id: generateId('te'), exerciseId: ex.id, defaultSets: 3, defaultReps: 10, restSeconds: 90, order: prev.length }]);
    ...
    <Modal visible={visible} transparent animationType="slide">
      <View style={styles.modalOverlay}>
        <View style={[styles.modalSheet, { maxHeight: '90%' }]}>
          ...
          <Text variant="subtitle" style={{ marginBottom: Spacing.md }}>{template ? 'Edit Template' : 'New Template'}</Text>
          <Text variant="label" style={{ marginBottom: 4 }}>TEMPLATE NAME</Text>
          <TextInput style={styles.calcInput} value={name} onChangeText={setName} placeholder="e.g. Push Day" placeholderTextColor={Colors.text.disabled} />
            ...
            {exercises.map((te) => {
              ...
              return (
                <View key={te.id} style={styles.tplExRow}>
                  <View style={{ flex: 1 }}><Text variant="bodyMedium">{ex?.name ?? te.exerciseId}</Text><Text variant="caption">{te.defaultSets} sets × {te.defaultReps} reps</Text></View>
                  <TouchableOpacity onPress={() => setExercises((p) => p.filter((e) => e.id !== te.id))}><MaterialCommunityIcons name="close" size={18} color={Colors.text.secondary} /></TouchableOpacity>
                </View>
              );
            })}
            {exercises.length === 0 && <Text variant="caption" color="secondary" style={{ textAlign: 'center', paddingVertical: Spacing.md }}>Tap + Add to add exercises</Text>}
          </ScrollView>
      ...
      <ExercisePickerModal visible={showExPicker} onSelect={addEx} onClose={() => setShowExPicker(false)} />
    </Modal>
  );
}
...
function ExercisesView() {
  ...
  const filtered = useMemo(() => EXERCISE_LIBRARY.filter((ex) => ex.name.toLowerCase().includes(search.toLowerCase()) && (!filterMuscle || ex.primaryMuscles.includes(filterMuscle))), [search, filterMuscle]);
  const handleAdd = (ex: typeof EXERCISE_LIBRARY[0]) => {
    if (!activeWorkout) {
      Alert.alert('No Active Workout', 'Start a workout first.', [{ text: 'Cancel', style: 'cancel' }, { text: 'Start Workout', onPress: () => { useWorkoutStore.getState().startWorkout('Quick Workout'); router.push('/workout/active'); } }]);
      return;
    }
    addExercise({ id: generateId('we'), exerciseId: ex.id, exercise: ex as Exercise, sets: [], restSeconds: 90 });
    router.push('/workout/active');
  };
  return (
    <View style={{ flex: 1 }}>
      <View style={styles.searchRow}>
        <View style={styles.searchInput}>
          ...
          <TextInput style={styles.searchText} placeholder="Search exercises..." placeholderTextColor={Colors.text.disabled} value={search} onChangeText={setSearch} />
          {search.length > 0 && <TouchableOpacity onPress={() => setSearch('')}><MaterialCommunityIcons name="close" size={16} color={Colors.text.secondary} /></TouchableOpacity>}
        </View>
      </View>
      <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.filterScroll} contentContainerStyle={{ gap: 8, padding: Spacing.md }}>
        {MUSCLE_GROUPS.slice(0, 10).map((mg) => (
          <TouchableOpacity key={mg.id} style={[styles.filterChip, filterMuscle === mg.id && styles.filterChipActive]} onPress={() => setFilterMuscle(filterMuscle === mg.id ? null : mg.id as MuscleGroup)}>
            <Text style={{ fontFamily: FontFamily.bodyMedium, fontSize: 12, color: filterMuscle === mg.id ? Colors.background.primary : Colors.text.secondary }}>{mg.label}</Text>
          </TouchableOpacity>
        ))}
      </ScrollView>
      {activeWorkout && (
        <View style={styles.addToWorkoutBanner}>
          ...
          <Text style={{ fontFamily: FontFamily.bodyMedium, fontSize: 12, color: BLUE, marginLeft: 6 }}>Tap any exercise to add it to your active workout</Text>
        </View>
      )}
      <FlatList
        data={filtered}
        ...
        renderItem={({ item }) => (
          <Card style={styles.exerciseCard}>
            <View style={styles.exerciseHeader}>
              <View style={{ flex: 1 }}><Text variant="bodyMedium">{item.name}</Text><Text variant="caption">{item.primaryMuscles.join(', ')} · {item.equipment}</Text></View>
              <View style={{ flexDirection: 'row', gap: Spacing.xs }}>
                {activeWorkout && <TouchableOpacity style={[styles.exActionBtn, { backgroundColor: `${BLUE}20`, borderColor: `${BLUE}40` }]} onPress={() => handleAdd(item)}><MaterialCommunityIcons name="plus" size={16} color={BLUE} /></TouchableOpacity>}
                <TouchableOpacity style={styles.exActionBtn} onPress={() => router.push(`/exercise/${item.id}`)}><MaterialCommunityIcons name="information-outline" size={16} color={Colors.text.secondary} /></TouchableOpacity>
              </View>
            </View>
          </Card>
        )}
        ListEmptyComponent={<EmptyState icon="dumbbell" title="No exercises found" description="Try a different search or filter." />}
      />
    </View>
  );
}
...
function HistoryView() {
  ...
  const workoutDates = useMemo(() => {
    ...
    recentWorkouts.forEach((w) => { if (w.completedAt) { const key = format(new Date(w.completedAt), 'yyyy-MM-dd'); map.set(key, [...(map.get(key) ?? []), w]); } });
    return map;
  }, [recentWorkouts]);
  ...
  return (
    <View style={{ flex: 1 }}>
      <View style={styles.calHeader}>
        <TouchableOpacity onPress={() => setCurrentMonth((m) => subMonths(m, 1))} hitSlop={{ top: 8, bottom: 8, left: 12, right: 12 }}><MaterialCommunityIcons name="chevron-left" size={24} color={Colors.text.primary} /></TouchableOpacity>
        <Text variant="bodyMedium">{format(currentMonth, 'MMMM yyyy')}</Text>
        <TouchableOpacity onPress={() => setCurrentMonth((m) => addMonths(m, 1))} hitSlop={{ top: 8, bottom: 8, left: 12, right: 12 }}><MaterialCommunityIcons name="chevron-right" size={24} color={Colors.text.primary} /></TouchableOpacity>
      </View>
      <View style={styles.calWeekRow}>
        {['Su', 'Mo', 'Tu', 'We', 'Th', 'Fr', 'Sa'].map((d) => <Text key={d} variant="caption" style={styles.calWeekLabel}>{d}</Text>)}
      </View>
      <ScrollView contentContainerStyle={{ padding: Spacing.md, paddingBottom: 100 }}>
        <View style={styles.calGrid}>
          {Array.from({ length: firstDayOfWeek }).map((_, i) => <View key={`e-${i}`} style={styles.calCell} />)}
          {days.map((day) => {
            const key = format(day, 'yyyy-MM-dd');
            const hasWorkout = workoutDates.has(key);
            const isToday = isSameDay(day, new Date());
            return (
              <TouchableOpacity key={key} style={[styles.calCell, isToday && styles.calCellToday]} onPress={() => { if (hasWorkout) setSelectedWorkout(workoutDates.get(key)![0]); }} activeOpacity={hasWorkout ? 0.7 : 1}>
                <Text style={[styles.calDayText, ...(isToday ? [{ color: Colors.background.primary as string, fontFamily: FontFamily.bodyBold as string }] : []), ...(hasWorkout && !isToday ? [{ color: BLUE as string }] : [])] as any}>{format(day, 'd')}</Text>
                {hasWorkout && <View style={[styles.calDot, { backgroundColor: isToday ? Colors.background.primary : BLUE }]} />}
              </TouchableOpacity>
            );
          })}
        </View>
        {recentWorkouts.length === 0 && <EmptyState icon="history" title="No workouts yet" description="Log your first workout to see it on the calendar." />}
              ...
              <TouchableOpacity key={w.id} onPress={() => setSelectedWorkout(w)}>
                <Card style={{ marginBottom: Spacing.sm }}>
                  <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                    <Text variant="caption" style={{ color: BLUE }}>{w.completedAt ? format(new Date(w.completedAt), 'EEE, MMM d') : '–'}</Text>
      ...
      <View style={styles.modalOverlay}>
        <View style={styles.modalSheet}>
          ...
          <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: Spacing.md }}>
            <Text variant="subtitle">{workout.completedAt ? format(new Date(workout.completedAt), 'EEE, MMM d') : workout.name}</Text>
            <TouchableOpacity onPress={onClose}><MaterialCommunityIcons name="close" size={22} color={Colors.text.secondary} /></TouchableOpacity>
          </View>
          <View style={{ flexDirection: 'row', gap: Spacing.lg, marginBottom: Spacing.md }}>
            {workout.durationSeconds != null && <View><Text variant="caption" color="secondary">Duration</Text><Text variant="bodyMedium">{formatDuration(workout.durationSeconds)}</Text></View>}
            {workout.totalVolumeKg != null && <View><Text variant="caption" color="secondary">Volume</Text><Text variant="bodyMedium">{Math.round(workout.totalVolumeKg).toLocaleString()} kg</Text></View>}
            <View><Text variant="caption" color="secondary">Exercises</Text><Text variant="bodyMedium">{workout.exercises.length}</Text></View>
          </View>
            ...
            {workout.exercises.map((ex) => {
              ...
              return (
                <View key={ex.id} style={{ marginBottom: Spacing.sm, paddingBottom: Spacing.sm, borderBottomWidth: 1, borderBottomColor: Colors.border.subtle }}>
                  ...
                  <Text variant="caption">{ex.sets.length} sets · {ex.sets.reduce((s, set) => s + set.reps, 0)} total reps</Text>
                </View>
              );
            })}
          </ScrollView>
...
function RecordsView() {
  ...
  if (prs.length === 0) return <EmptyState icon="trophy-outline" title="No PRs yet" description="Complete workouts to set personal records." />;
  return (
    <FlatList
      data={[...prs].sort((a, b) => b.e1rm - a.e1rm).slice(0, 20)}
          ...
          <Card style={{ marginBottom: Spacing.xs, borderLeftWidth: 3, borderLeftColor: BLUE }}>
            <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
              ...
              <View style={{ alignItems: 'flex-end' }}>
                <Text variant="mono" style={{ fontSize: 18, color: BLUE }}>{pr.e1rm.toFixed(1)} kg</Text>
...
function ExercisePickerModal({ visible, onSelect, onClose }: { visible: boolean; onSelect: (ex: typeof EXERCISE_LIBRARY[0]) => void; onClose: () => void }) {
  const [search, setSearch] = useState('');
  const [filterMuscle, setFilterMuscle] = useState<MuscleGroup | null>(null);
  const filtered = useMemo(() => EXERCISE_LIBRARY.filter((e) => e.name.toLowerCase().includes(search.toLowerCase()) && (!filterMuscle || e.primaryMuscles.includes(filterMuscle))), [search, filterMuscle]);
  return (
      ...
      <View style={styles.modalOverlay}>
        <View style={[styles.modalSheet, { height: '85%' }]}>
          ...
          <View style={[styles.searchInput, { marginBottom: Spacing.sm }]}>
            <MaterialCommunityIcons name="magnify" size={18} color={Colors.text.secondary} />
            <TextInput style={styles.searchText} placeholder="Search..." placeholderTextColor={Colors.text.disabled} value={search} onChangeText={setSearch} autoFocus />
          </View>
          <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ maxHeight: 40, flexGrow: 0, marginBottom: Spacing.sm }} contentContainerStyle={{ gap: 6, paddingRight: Spacing.md }}>
            {MUSCLE_GROUPS.slice(0, 10).map((mg) => (
              <TouchableOpacity key={mg.id} style={[styles.filterChip, filterMuscle === mg.id && styles.filterChipActive]} onPress={() => setFilterMuscle(filterMuscle === mg.id ? null : mg.id as MuscleGroup)}>
                <Text style={{ fontFamily: FontFamily.bodyMedium, fontSize: 11, color: filterMuscle === mg.id ? Colors.background.primary : Colors.text.secondary }}>{mg.label}</Text>
              </TouchableOpacity>
            ))}
          </ScrollView>
          <FlatList data={filtered} keyExtractor={(e) => e.id} style={{ flex: 1 }} renderItem={({ item }) => (
...
function PlateCalculator({ visible, onClose }: { visible: boolean; onClose: () => void }) {
  ...
  const result = useMemo(() => { const t = parseFloat(target) || 0; const b = parseFloat(barbell) || 20; if (t <= b) return { plates: [], remainder: 0 }; return calculatePlates(t, b); }, [target, barbell]);
  return (
    <Modal visible={visible} transparent animationType="slide">
      <View style={styles.modalOverlay}>
        <View style={styles.modalSheet}>
          ...
          <View style={styles.calcRow}>
            <View style={{ flex: 1 }}><Text variant="label" style={{ marginBottom: 4 }}>Target (kg)</Text><TextInput style={styles.calcInput} value={target} onChangeText={setTarget} keyboardType="decimal-pad" placeholderTextColor={Colors.text.disabled} /></View>
            <View style={{ flex: 1, marginLeft: Spacing.md }}><Text variant="label" style={{ marginBottom: 4 }}>Bar (kg)</Text><TextInput style={styles.calcInput} value={barbell} onChangeText={setBarbell} keyboardType="decimal-pad" placeholderTextColor={Colors.text.disabled} /></View>
          </View>
          <Text variant="label" style={{ marginTop: Spacing.md, marginBottom: Spacing.sm }}>PLATES PER SIDE</Text>
          {result.plates.length === 0 ? <Text variant="body" color="secondary">Enter a weight above bar weight.</Text> : (
            <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: Spacing.xs }}>
              {result.plates.map((p: number, i: number) => <View key={i} style={[styles.plateChip, { borderColor: `${BLUE}40` }]}><Text style={{ fontFamily: FontFamily.monoBold, fontSize: 14, color: BLUE }}>{p}</Text><Text variant="caption">kg</Text></View>)}
            </View>
          )}
