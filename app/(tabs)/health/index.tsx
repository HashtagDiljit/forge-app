...
function SparklineChart({ data, color = RED, height = 100 }: {
  ...
  if (data.length < 2) {
    return (
      <View style={{ height, alignItems: 'center', justifyContent: 'center' }}>
        <Text variant="caption" color="secondary">Not enough data</Text>
      </View>
    );
  }
  ...
  const yLabels = [
    { y: yPt(minY), label: minY.toFixed(minY < 10 ? 1 : 0) },
    { y: yPt(maxY), label: maxY.toFixed(maxY < 10 ? 1 : 0) },
  ];

  const xLabels = [
    { x: xPt(minX), label: format(new Date(minX), 'M/d') },
    { x: xPt(maxX), label: format(new Date(maxX), 'M/d') },
  ];
  ...
  return (
    <Svg width={chartW} height={height}>
      ...
      {yLabels.map((l, i) => (
        <SvgText key={i} x={pad.left - 4} y={l.y + 4} textAnchor="end" fontSize={9} fill={Colors.text.disabled}>
          {l.label}
        </SvgText>
      ))}

      {xLabels.map((l, i) => (
        <SvgText key={i} x={l.x} y={height - 2} textAnchor="middle" fontSize={9} fill={Colors.text.disabled}>
          {l.label}
        </SvgText>
      ))}
    </Svg>
  ...
  return (
    <View style={styles.timeToggle}>
      {([7, 30, 90] as TimeRange[]).map((v) => (
        <TouchableOpacity
          key={v}
          style={[styles.timeToggleBtn, value === v && { backgroundColor: RED }]}
          onPress={() => onChange(v)}
        >
          <Text style={{ fontFamily: FontFamily.bodyMedium, fontSize: 11, color: value === v ? '#fff' : Colors.text.secondary }}>
            {v}d
          </Text>
        </TouchableOpacity>
      ))}
...
function MetricGraphCard({ title, metricType, unit, onAdd, formatVal }: {
  title: string;
  ...
  const history = getMetricHistory(metricType, range);

  const chartData = useMemo(() =>
    history
      .slice()
      .sort((a, b) => a.recordedAt - b.recordedAt)
      .map((m) => ({ x: m.recordedAt, y: m.value })),
    [history]
  );
    ...
    <Card style={styles.graphCard}>
      <View style={styles.graphCardHeader}>
        <View style={{ flex: 1 }}>
          ...
          <View style={{ flexDirection: 'row', alignItems: 'baseline', gap: 4, marginTop: 2 }}>
            <Text style={{ fontFamily: FontFamily.display, fontSize: 32, color: Colors.text.primary }}>
              {displayVal}
            </Text>
            <Text variant="caption" color="secondary">{unit}</Text>
          </View>
          {latest && (
            <Text variant="caption" color="secondary">{format(new Date(latest.recordedAt), 'MMM d, yyyy')}</Text>
          )}
        </View>
        <View style={{ alignItems: 'flex-end', gap: Spacing.sm }}>
          <TouchableOpacity style={styles.addCircleBtn} onPress={onAdd}>
            <MaterialCommunityIcons name="plus" size={18} color={RED} />
          </TouchableOpacity>
      ...
      {chartData.length >= 2 ? (
        <View style={{ marginTop: Spacing.sm }}>
          <SparklineChart data={chartData} color={RED} height={100} />
        </View>
      ) : (
        <View style={{ marginTop: Spacing.sm, alignItems: 'center', paddingVertical: Spacing.md }}>
          <Text variant="caption" color="secondary">Log at least 2 entries to see the trend</Text>
        </View>
      )}
    </Card>
  );
    ...
    <ScrollView contentContainerStyle={styles.sectionContent} showsVerticalScrollIndicator={false}>
      ...
      {bmi && (
        <Card style={{ marginBottom: Spacing.md, flexDirection: 'row', alignItems: 'center', gap: Spacing.md }}>
          <MaterialCommunityIcons name="scale" size={20} color={RED} />
          <View>
            <Text variant="label" color="secondary">BMI</Text>
      ...
      <MetricGraphCard
        ...
        metricType="body_fat"
        unit="%"
        onAdd={() => setShowBodyFatModal(true)}
        formatVal={(v) => v.toFixed(1)}
      />
      ...
      <Card style={styles.simpleCard} onPress={() => setShowMeasurementsModal(true)}>
        <View style={styles.simpleCardRow}>
          ...
          <View style={{ flex: 1, marginLeft: Spacing.sm }}>
            ...
            <Text variant="caption" color="secondary">Waist, chest, arms, hips…</Text>
          </View>
          <TouchableOpacity style={styles.addCircleBtn} onPress={() => setShowMeasurementsModal(true)}>
            <MaterialCommunityIcons name="plus" size={18} color={RED} />
          </TouchableOpacity>
        </View>
      </Card>
      ...
      <LogMetricModal
        ...
        onSave={(v) => { addMetric({ id: generateId('m'), type: 'weight', value: v, unit: weightUnit, recordedAt: Date.now(), source: 'manual' }); setShowWeightModal(false); }}
      />
      <LogMetricModal
        ...
        onSave={(v) => { addMetric({ id: generateId('m'), type: 'body_fat', value: v, unit: '%', recordedAt: Date.now(), source: 'manual' }); setShowBodyFatModal(false); }}
      />
...
function CardioSection() {
  ...
  const metrics: { type: HealthMetricType; label: string; unit: string; icon: string; placeholder: string }[] = [
    ...
    { type: 'distance', label: 'Distance', unit: 'km', icon: 'map-marker-distance', placeholder: '5.0' },
    ...
    <ScrollView contentContainerStyle={styles.sectionContent} showsVerticalScrollIndicator={false}>
      {metrics.map((m) => (
        <MetricGraphCard
          ...
          metricType={m.type}
          unit={m.unit}
          onAdd={() => setShowModal(m.type)}
          formatVal={m.type === 'steps' ? (v) => Math.round(v).toLocaleString() : undefined}
        />
      ))}
...
function SleepSection() {
  ...
  const chartData = useMemo(() =>
    filtered
      ...
      .map((l) => ({ x: l.wakeTime, y: l.totalMinutes / 60 })),
    [filtered]
  );
  ...
  return (
    <ScrollView contentContainerStyle={styles.sectionContent} showsVerticalScrollIndicator={false}>
      <Card style={styles.graphCard}>
        <View style={styles.graphCardHeader}>
          <View style={{ flex: 1 }}>
            ...
            <View style={{ flexDirection: 'row', alignItems: 'baseline', gap: 4, marginTop: 2 }}>
              <Text style={{ fontFamily: FontFamily.display, fontSize: 32, color: Colors.text.primary }}>
                {latestSleep ? (latestSleep.totalMinutes / 60).toFixed(1) : '--'}
              </Text>
          ...
          <View style={{ alignItems: 'flex-end', gap: Spacing.sm }}>
            <TouchableOpacity style={styles.addCircleBtn} onPress={() => setShowModal(true)}>
              <MaterialCommunityIcons name="plus" size={18} color={RED} />
            </TouchableOpacity>
            <TimeRangeToggle value={range} onChange={setRange} />
          </View>
      ...
      {sleepLogs.length === 0 && (
        <EmptyState icon="sleep" title="No sleep logs" description="Log your sleep to track trends." />
      )}
    ...
    <ScrollView contentContainerStyle={styles.sectionContent} showsVerticalScrollIndicator={false}>
      <Card style={styles.graphCard}>
        ...
        <View style={styles.progressBar}>
          <View style={[styles.progressFill, { width: `${Math.min(totalCals / targets.calorieTarget, 1) * 100}%`, backgroundColor: RED }]} />
        </View>
      </Card>
      ...
      <Card style={styles.graphCard} onPress={() => setShowWaterModal(true)}>
        <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 4 }}>
          ...
          <TouchableOpacity style={styles.addCircleBtn} onPress={() => setShowWaterModal(true)}>
            <MaterialCommunityIcons name="plus" size={18} color={RED} />
          </TouchableOpacity>
        </View>
      ...
      <View style={{ flexDirection: 'row', gap: Spacing.sm, marginBottom: Spacing.md }}>
        <Button
          ...
          style={{ flex: 1 }}
          leftIcon={<MaterialCommunityIcons name="food" size={18} color={Colors.background.primary} />}
        />
        <Button
          label="Log Water"
          ...
          variant="secondary"
          style={{ flex: 1 }}
          leftIcon={<MaterialCommunityIcons name="water" size={18} color={Colors.text.primary} />}
        />
      </View>
          ...
          <View style={styles.modalSheet}>
            ...
            <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: Spacing.sm, marginBottom: Spacing.md }}>
              {[150, 250, 330, 500].map((ml) => (
                <TouchableOpacity key={ml} style={[styles.quickWater, waterAmount === ml.toString() && { backgroundColor: `${RED}20`, borderColor: RED }]} onPress={() => setWaterAmount(ml.toString())}>
                  <Text style={{ fontFamily: FontFamily.bodyMedium, fontSize: 13, color: waterAmount === ml.toString() ? RED : Colors.text.primary }}>{ml} ml</Text>
                </TouchableOpacity>
              ))}
            </View>
...
function WellnessSection() {
  ...
  const saveWellness = () => {
    addMoodLog({ id: generateId('mood'), mood, energy, stress, notes: notes.trim() || undefined, loggedAt: Date.now() });
    setNotes('');
  };
        ...
        <Card style={styles.graphCard}>
          ...
          <View style={styles.moodRow}>
            {([1, 2, 3, 4, 5] as const).map((m) => (
              <TouchableOpacity key={m} style={[styles.moodButton, mood === m && styles.moodButtonActive]} onPress={() => setMood(m)}>
    ...
    <View style={{ marginBottom: Spacing.md }}>
      ...
      <View style={{ flexDirection: 'row', gap: 4 }}>
        {Array.from({ length: 10 }, (_, i) => i + 1).map((v) => (
          <TouchableOpacity key={v} style={[styles.sliderDot, v <= value && { backgroundColor: color }]} onPress={() => onChange(v)} />
        ))}
      </View>
    </View>
  );
...
function LogMetricModal({ visible, title, metricType, unit, placeholder, onClose, onSave }: {
  visible: boolean; title?: string; metricType: HealthMetricType; unit: string; placeholder: string;
  onClose: () => void; onSave: (value: number) => void;
}) {
...
function MeasurementsModal({ visible, onClose }: { visible: boolean; onClose: () => void }) {
  ...
  const save = () => {
    Object.entries(values).forEach(([type, val]) => {
      if (val && parseFloat(val) > 0) addMetric({ id: generateId('m'), type: type as HealthMetricType, value: parseFloat(val), unit: 'cm', recordedAt: Date.now(), source: 'manual' });
    });
    onClose();
  };
      ...
      <View style={styles.modalOverlay}>
        <View style={styles.modalSheet}>
          ...
          <ScrollView style={{ maxHeight: 360 }}>
            {measurements.map((m) => (
              <Input key={m.type} label={m.label} value={values[m.type] ?? ''} onChangeText={(v) => setValues((prev) => ({ ...prev, [m.type]: v }))} keyboardType="decimal-pad" placeholder="–" />
            ))}
          </ScrollView>
          <View style={{ flexDirection: 'row', gap: Spacing.md, marginTop: Spacing.md }}>
            <Button label="Cancel" onPress={onClose} variant="secondary" style={{ flex: 1 }} />
