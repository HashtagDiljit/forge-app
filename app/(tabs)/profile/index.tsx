...
function AvatarSection({ onEditPress }: { onEditPress: () => void }) {
  ...
  return (
    <View style={styles.avatarSection}>
      ...
      <Text variant="title" style={{ marginTop: Spacing.sm }}>{user?.name || 'Set your name'}</Text>
      {user?.bio ? (
        <Text variant="caption" color="secondary" style={{ marginTop: 4, textAlign: 'center', maxWidth: 260 }}>{user.bio}</Text>
      ) : null}
    </View>
  );
}
...
function BioEditForm({ onDone }: { onDone: () => void }) {
  ...
  const save = () => {
    updateUser({ name: name.trim() || 'Unknown', age: parseInt(age) || 0, heightCm: parseFloat(heightCm) || 0, weightKg: parseFloat(weightKg) || 0, bio: bio.trim() });
    onDone();
  };
  return (
    <Card style={styles.sectionCard}>
      ...
      <Input label="Bio (optional)" value={bio} onChangeText={setBio} placeholder="Tell us a bit about yourself..." multiline numberOfLines={3} />
    ...
    return (
      <Card style={styles.sectionCard}>
        <View style={styles.emptyBioRow}>
          ...
          <Text variant="caption" color="secondary" style={{ flex: 1, marginLeft: Spacing.sm }}>Tap to set up your profile</Text>
...
function SettingsSection() {
  const { theme, setTheme, unitSystem, setUnitSystem, notificationsEnabled, setNotificationsEnabled, habitRemindersEnabled, setHabitRemindersEnabled, workoutRemindersEnabled, setWorkoutRemindersEnabled, waterRemindersEnabled, setWaterRemindersEnabled, nutritionTargets, setNutritionTargets, clearAll } = useSettingsStore();
  ...
  const saveNutrition = () => {
    setNutritionTargets({ calorieTarget: parseInt(calTarget) || nutritionTargets.calorieTarget, waterMl: parseInt(waterTarget) || nutritionTargets.waterMl, proteinG: parseInt(proteinTarget) || nutritionTargets.proteinG });
  ...
  return (
    <>
      ...
      <Card style={styles.sectionCard}>
        <SettingRow icon="theme-light-dark" label="Dark Mode" description={theme === 'dark' ? 'Currently using dark theme' : 'Currently using light theme'}>
          <Switch value={theme === 'dark'} onValueChange={(v) => setTheme(v ? 'dark' : 'light')} trackColor={{ true: Colors.accent.primary, false: Colors.background.elevated }} thumbColor={Colors.text.primary} />
        </SettingRow>
        <SettingDivider />
        <SettingRow icon="ruler" label="Units" description={unitSystem === 'metric' ? 'Metric (kg / cm)' : 'Imperial (lbs / ft)'}>
          <View style={styles.togglePill}>
            <TouchableOpacity style={[styles.toggleOption, unitSystem === 'metric' && styles.toggleOptionActive]} onPress={() => setUnitSystem('metric')}>
            ...
            <TouchableOpacity style={[styles.toggleOption, unitSystem === 'imperial' && styles.toggleOptionActive]} onPress={() => setUnitSystem('imperial')}>
      ...
      <Card style={styles.sectionCard}>
        <SettingRow icon="bell-outline" label="All Notifications" description="Master toggle for all alerts">
          <Switch value={notificationsEnabled} onValueChange={setNotificationsEnabled} trackColor={{ true: Colors.accent.primary, false: Colors.background.elevated }} thumbColor={Colors.text.primary} />
        </SettingRow>
        <SettingDivider />
        <SettingRow icon="check-circle-outline" label="Habit Reminders" description="Daily habit check-in alerts">
          <Switch value={habitRemindersEnabled && notificationsEnabled} onValueChange={setHabitRemindersEnabled} disabled={!notificationsEnabled} trackColor={{ true: Colors.accent.primary, false: Colors.background.elevated }} thumbColor={Colors.text.primary} />
        </SettingRow>
        <SettingDivider />
        <SettingRow icon="dumbbell" label="Workout Reminders" description="Session schedule alerts">
          <Switch value={workoutRemindersEnabled && notificationsEnabled} onValueChange={setWorkoutRemindersEnabled} disabled={!notificationsEnabled} trackColor={{ true: Colors.accent.primary, false: Colors.background.elevated }} thumbColor={Colors.text.primary} />
        </SettingRow>
        <SettingDivider />
        <SettingRow icon="water-outline" label="Water Reminders" description="Hydration nudges throughout the day">
          <Switch value={waterRemindersEnabled && notificationsEnabled} onValueChange={setWaterRemindersEnabled} disabled={!notificationsEnabled} trackColor={{ true: Colors.accent.primary, false: Colors.background.elevated }} thumbColor={Colors.text.primary} />
        </SettingRow>
      </Card>
      ...
      <Card style={styles.sectionCard}>
        <SettingRow icon="information-outline" label="FORGE" description="Version 1.0.0 · Built with Expo"><View /></SettingRow>
        <SettingDivider />
        <SettingRow icon="shield-lock-outline" label="Privacy" description="Your data stays on your device"><View /></SettingRow>
      </Card>
      <Text variant="label" color="secondary" style={styles.groupLabel}>DATA</Text>
      <Card style={styles.sectionCard}>
        <TouchableOpacity onPress={() => Alert.alert('Clear All Data', 'This will permanently delete all your workouts, health metrics, habits, and goals. This cannot be undone.', [{ text: 'Cancel', style: 'cancel' }, { text: 'Clear Everything', style: 'destructive', onPress: () => { clearAll(); useHealthStore.getState().clearAll(); useWorkoutStore.setState({ recentWorkouts: [], templates: [], personalRecords: [], lastSetsByExercise: {}, activeWorkout: null }); } }])}>
...
const styles = StyleSheet.create({
  ...
  avatarFallback: { width: 88, height: 88, borderRadius: 44, backgroundColor: Colors.background.elevated, alignItems: 'center', justifyContent: 'center', borderWidth: 2, borderColor: Colors.accent.primary },
  avatarInitials: { fontFamily: FontFamily.display, fontSize: 28, color: Colors.accent.primary },
  avatarEditBtn: { position: 'absolute', bottom: 0, right: 0, width: 26, height: 26, borderRadius: 13, backgroundColor: Colors.accent.primary, alignItems: 'center', justifyContent: 'center', borderWidth: 2, borderColor: Colors.background.primary },
  ...
  togglePill: { flexDirection: 'row', backgroundColor: Colors.background.card, borderRadius: Radius.sm, borderWidth: 1, borderColor: Colors.border.subtle, overflow: 'hidden' },
